<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Pendaftaran_model extends CI_Model
{

    public $table = 'pendaftaran';
    public $id = 'id_pendaftaran';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id_pendaftaran', $q);
	$this->db->or_like('no_pendaftaran', $q);
	$this->db->or_like('nama_lengkap', $q);
	$this->db->or_like('tempat', $q);
	$this->db->or_like('tgl_lahir', $q);
	$this->db->or_like('alamat', $q);
	$this->db->or_like('rt', $q);
	$this->db->or_like('rw', $q);
	$this->db->or_like('no_rumah', $q);
	$this->db->or_like('kode_pos', $q);
	$this->db->or_like('no_telp', $q);
	$this->db->or_like('tempat_tinggal', $q);
	$this->db->or_like('status_kawin', $q);
	$this->db->or_like('jenis_pekerjaan', $q);
	$this->db->or_like('kewarganegaraan', $q);
	$this->db->or_like('agama', $q);
	$this->db->or_like('hobi', $q);
	$this->db->or_like('slta', $q);
	$this->db->or_like('jurusan', $q);
	$this->db->or_like('no_sttb', $q);
	$this->db->or_like('tgl_sttb', $q);
	$this->db->or_like('tempat_sttb', $q);
	$this->db->or_like('pilihan_studi', $q);
	$this->db->or_like('foto', $q);
	$this->db->or_like('tgl_buat', $q);
	$this->db->or_like('jam_buat', $q);
	$this->db->or_like('tahun_lulus', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id_pendaftaran', $q);
	$this->db->or_like('no_pendaftaran', $q);
	$this->db->or_like('nama_lengkap', $q);
	$this->db->or_like('tempat', $q);
	$this->db->or_like('tgl_lahir', $q);
	$this->db->or_like('alamat', $q);
	$this->db->or_like('rt', $q);
	$this->db->or_like('rw', $q);
	$this->db->or_like('no_rumah', $q);
	$this->db->or_like('kode_pos', $q);
	$this->db->or_like('no_telp', $q);
	$this->db->or_like('tempat_tinggal', $q);
	$this->db->or_like('status_kawin', $q);
	$this->db->or_like('jenis_pekerjaan', $q);
	$this->db->or_like('kewarganegaraan', $q);
	$this->db->or_like('agama', $q);
	$this->db->or_like('hobi', $q);
	$this->db->or_like('slta', $q);
	$this->db->or_like('jurusan', $q);
	$this->db->or_like('no_sttb', $q);
	$this->db->or_like('tgl_sttb', $q);
	$this->db->or_like('tempat_sttb', $q);
	$this->db->or_like('pilihan_studi', $q);
	$this->db->or_like('foto', $q);
	$this->db->or_like('tgl_buat', $q);
	$this->db->or_like('jam_buat', $q);
	$this->db->or_like('tahun_lulus', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }

}

/* End of file Pendaftaran_model.php */
/* Location: ./application/models/Pendaftaran_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2018-05-09 04:46:28 */
/* http://harviacode.com */